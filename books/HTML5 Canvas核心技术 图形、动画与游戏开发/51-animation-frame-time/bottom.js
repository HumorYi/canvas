...
;(() => {
  ...
  const animateButton = document.querySelector('#bottomAnimateButton')
  ...
  const updateFps = () => {
    ...
    if (!paused) {
      bottomContext.fillStyle = 'cornflowerblue'
      bottomContext.fillText(lastFpsUpdate.value.toFixed() + ' fps', 50, 48)
    }
  }
  ...
  const animate = time => {
    ...
    if (!paused) {
      ...
      drawBackground(bottomContext, width, height)
      timeBasedMotionCheckbox.checked ? updateTimeBased(time) : update(discs, bottomContext, width, height)
    }
  ...
  bottomContext.font = '36px Helvetica'
  drawBackground(bottomContext, width, height)
